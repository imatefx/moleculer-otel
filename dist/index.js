...
__export(index_exports, {
  ...
  getAllBaggage: () => getAllBaggage,
  getBaggage: () => getBaggage,
  getBaggageValue: () => getBaggageValue,
...
function injectContext(ctx, carrier) {
  import_api.propagation.inject(ctx, carrier, textMapSetter);
}
function extractContext(ctx, carrier) {
  return import_api.propagation.extract(ctx, carrier, textMapGetter);
}
...
function getBaggage() {
  return import_api.propagation.getBaggage(import_api.context.active());
}
...
function withBaggage(entries, parentContext) {
  const ctx = parentContext ?? import_api.context.active();
  const currentBaggage = import_api.propagation.getBaggage(ctx) ?? import_api.propagation.createBaggage();
  ...
  return import_api.propagation.setBaggage(ctx, newBaggage);
}
...
var MoleculerMetrics = class {
  ...
  constructor(options = {}) {
    ...
    const meter = import_api3.metrics.getMeter(METER_NAME);
...
function initOTel(options = {}) {
  ...
  const resourceAttributes = {
    [import_semantic_conventions4.ATTR_SERVICE_NAME]: serviceName,
    [import_semantic_conventions4.ATTR_SERVICE_VERSION]: serviceVersion,
...
function getTraceLogContext() {
  const span = import_api19.trace.getSpan(import_api19.context.active());
  if (!span) {
    return void 0;
  }
...
function getTracer(name = "moleculer-otel") {
  return import_api20.trace.getTracer(name);
}
